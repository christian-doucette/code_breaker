...
# assumes text only includes uppercase letters and spaces
def break_substitution(cipher_text: str, num_trials: int = 15, n_swaps: int = 1000) -> str:
    # number of letters in the guesses so far
    best_score_overall = -1
    best_score_func    = {letter: letter for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ"}

    for trial_num in range(num_trials):
        print(f'Trial {trial_num}')
        this_trial_best_score = -1
        this_trial_func       = encrypt.get_random_substitution()


        for _ in range(n_swaps):
            swap = random.sample(range(26), k=2)
            swapped_func = this_trial_func.copy()
            swapped_func[chr(swap[0] + 65)] = this_trial_func[chr(swap[1] + 65)]
            swapped_func[chr(swap[1] + 65)] = this_trial_func[chr(swap[0] + 65)]

            swapped_func_score = calculate_score(encrypt.encrypt_substitution(cipher_text, swapped_func), 5)
            if swapped_func_score > this_trial_best_score:
                this_trial_func          = swapped_func
                this_trial_best_score    = swapped_func_score

        if this_trial_best_score > best_score_overall:
            best_score_overall = this_trial_best_score
            best_score_func    = this_trial_func

    return encrypt.encrypt_substitution(cipher_text, best_score_func)
...
my_cipher_text = encrypt.encrypt_substitution("MY NAME IS CHRISTIAN DOUCETTE AND I APPROVE THIS MESSAGE", encrypt.get_random_substitution())
